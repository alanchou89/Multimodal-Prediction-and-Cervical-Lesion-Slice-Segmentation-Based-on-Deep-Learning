...
from my_mae_model_2_NL import fusion_model_mae_2
...
def prediction(all_data,v_model,val_id,patient_diagnosis_type,args):
    ...
    val_pre_cli = {}
    iter = 0
    logits_all, logits_img, logits_cli, logits_imgN, logits_imgL = [], [], [], [], []
    ...
    with torch.no_grad():
        for i_batch, id in enumerate(val_id):
            ...
            all_logits,imgN_logits,imgL_logits = v_model(graph,args.train_use_type,use_type_eopch,mix=args.mix)
    ...
    return loss, accuracy_all, accuracy_imgN, accuracy_imgL
###==========================================================================================###
...
def train_a_epoch(model, train_data,all_data, patient_diagnosis_type, batch_size, optimizer, epoch, format_of_coxloss, args):
    ...
    for i_batch,id in enumerate(train_data):
        ...
        out_pre,out_fea,out_att,fea_dict,\
            all_logits, imgN_logits, imgL_logits = model(graph,use_type_eopch,use_type_eopch,mask,mix=args.mix)
        ...
        if iter % batch_size == 0 or i_batch == len(train_data)-1:
            ...
            if format_of_coxloss == 'multi':
                ...
                loss_surv = loss_surv_imgN + loss_surv_imgL + loss_surv_all
                a = loss_surv_all  
            loss = loss_surv  
    ...
    return total_loss, accuracy_all, accuracy_imgN, accuracy_imgL
###==========================================================================================###
    ...
    for seed in range(start_seed, start_seed + repeat_num):
        ...
        for train_index, test_index in kf.split(patient_ids,kf_label):
            ...
            for epoch in range(epochs):
                ...
                train_loss, train_accuracy_all, train_accuracy_imgN, train_accuracy_imgL = train_a_epoch(model,train_data,all_data,patient_diagnosis_type,batch_size,optimizer,epoch, format_of_coxloss, args)
                ...
                print("epoch:{:2d}, train_accuracy_imgN:{:.4f}, train_accuracy_imgL:{:.4f}".format(epoch, train_accuracy_imgN, train_accuracy_imgL)) 
