...
from my_mae_model_2 import fusion_model_mae_2
...
def prediction(all_data,v_model,val_id,patient_diagnosis_type,args):
    ...
    with torch.no_grad():
        for i_batch, id in enumerate(val_id):
            ...
            all_logits,img_logits,cli_logits = v_model(graph,args.train_use_type,use_type_eopch,mix=args.mix)
    ...
    loss_img = criterion(logits_img, val_diagnosis_T)
    loss_cli = criterion(logits_cli, val_diagnosis_T)
    loss = loss_all

    # 轉換列表為 NumPy 數組
    predicted_labels_all = np.concatenate(predicted_labels_all)
    predicted_labels_img = np.concatenate(predicted_labels_img)
    ...
    accuracy_img = accuracy_score(val_diagnosis_T_cpu, predicted_labels_img)
    accuracy_cli = accuracy_score(val_diagnosis_T_cpu, predicted_labels_cli)

    return loss, accuracy_all, accuracy_img, accuracy_cli
###===========================================================================###
...
def train_a_epoch(model, train_data,all_data, patient_diagnosis_type, batch_size, optimizer, epoch, format_of_coxloss, args):
    ...
    for i_batch,id in enumerate(train_data):
        ...
        out_pre,out_fea,out_att,fea_dict,\
            all_logits, img_logits, cli_logits = model(graph,use_type_eopch,use_type_eopch,mask,mix=args.mix)
        ...
        img_logits = img_logits.unsqueeze(0) #加的
        ...
        if iter % batch_size == 0 or i_batch == len(train_data)-1:
            ...
            if format_of_coxloss == 'multi':
                loss_surv_img = loss_img * 0.3
                loss_surv_cli = loss_cli * 0.2
                loss_surv_all = loss_all * 1.0
                loss_surv = loss_surv_img + loss_surv_cli + loss_surv_all
                a = loss_surv_all  
            loss = loss_surv  
    ...
    accuracy_img = accuracy_score(diagnosis_all, predicted_labels_img)
    accuracy_cli = accuracy_score(diagnosis_all, predicted_labels_cli)

    return total_loss, accuracy_all, accuracy_img, accuracy_cli
###===========================================================================###
    ...
    for seed in range(start_seed,start_seed+repeat_num):
        ...
        for train_index, test_index in kf.split(patient_ids,kf_label):
            ...
            for epoch in range(epochs):
                ...
                train_loss, train_accuracy_all, train_accuracy_img, train_accuracy_cli = train_a_epoch(model,train_data,all_data,patient_diagnosis_type,batch_size,optimizer,epoch, format_of_coxloss, args)
                ...
                print("epoch:{:2d}, train_accuracy_img:{:.4f}, train_accuracy_cli:{:.4f}".format(epoch, train_accuracy_img, train_accuracy_cli)) 
