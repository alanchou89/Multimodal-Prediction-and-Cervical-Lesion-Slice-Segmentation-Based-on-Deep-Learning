    ...
    for seed in range(start_seed, start_seed + repeat_num):
        ...
        for train_index, test_index in kf.split(patient_ids,kf_label):
            ...
            for epoch in range(epochs):
                ...
                test_loss,test_accuracy_all,test_accuracy_imgN,test_accuracy_imgA,test_accuracy_imgL,test_accuracy_cli, = prediction(all_data,model,test_data,patient_diagnosis_type, args)  
                val_loss,val_accuracy_all,val_accuracy_imgN,val_accuracy_imgA,val_accuracy_imgL,val_accuracy_cli, = prediction(all_data,model,val_data,patient_diagnosis_type, args)
                if epoch == 0:
                    print("\n")
                ...
                print("epoch:{:2d}, train_accuracy_imgN:{:.4f}, train_accuracy_imgL:{:.4f}, train_accuracy_cli:{:.4f}".format(epoch,train_accuracy_imgN,train_accuracy_imgL,train_accuracy_cli)) 
                ...
                if val_accuracy_all > best_val_ac and epoch>=1 :
                    ...
                    print("更新最佳 Impression:{:.4f}".format(val_accuracy_impression))
                    t_model = copy.deepcopy(model)
