...
from my_mae_model_three import fusion_model_mae_2
...
def prediction(all_data, v_model, val_id, patient_diagnosis_type, args):
    ...
    with torch.no_grad():
        for i_batch, id in enumerate(val_id):
            ...
            all_logits, imgN_logits, imgA_logits, imgL_logits, cli_logits \
            ...
            if imgN_logits == None:
                all_logits = all_logits.unsqueeze(0)  # 將尺寸從 [4] 轉換為 [1, 4]
                imgA_logits = imgA_logits.unsqueeze(0)
                imgL_logits = imgL_logits.unsqueeze(0)
                ...
                probabilities_imgL = F.softmax(imgL_logits, dim=1) 
                ...
                logits_imgL.append(imgL_logits)
                logits_cli.append(cli_logits)
    ...
    #print("val_diagnosis_T: ", val_diagnosis_T) #[batch_size, 4]

    val_Impression_T = np.asarray(val_Impression_T)
    ...
    #loss_imgL = criterion(logits_imgL, val_diagnosis_T)
    #loss_cli = criterion(logits_cli, val_diagnosis_T)
    loss = loss_all 
...
def train_a_epoch(model, train_data, all_data, patient_diagnosis_type, batch_size, optimizer, epoch, format_of_coxloss, args):
    ...
    for i_batch,id in enumerate(train_data):
        ...
        if imgN_logits == None:
            ...
            imgL_logits = imgL_logits.unsqueeze(0) #加的
        ...
        if iter % batch_size == 0 or i_batch == len(train_data) - 1:
            ...
            if format_of_coxloss == 'multi':
                ...
                loss_surv_cli = ( loss_cli ) * 0.2
                loss_surv_all = loss_all * 1.0
                loss_surv = loss_surv_imgN + loss_surv_imgA + loss_surv_imgL + loss_surv_cli + loss_surv_all
                #print("loss_imgN: ", loss_surv_imgN," loss_imgA: ", loss_surv_imgA," loss_imgL: ", loss_surv_imgL," loss_cli: ", loss_surv_cli," loss_all: ", loss_surv_all) 
                a = loss_surv_all
            loss = loss_surv  
    ...
    Impression_all = np.asarray(Impression_all)
...
def main(args): 
    ...
    all_data=joblib.load(r'圖結構資料位置')
    ...
    for seed in range(start_seed, start_seed + repeat_num):
        ...
        # 第二步
        kf = StratifiedKFold(n_splits= 10,shuffle=True,random_state = seed)  # n_splits= 5
        for train_index, test_index in kf.split(patient_ids,kf_label):
            ...
            if args.if_fit_split:
                ...
                train_data, val_data ,_ , _ = train_test_split(t_train_data,t_train_data,test_size=0.11,random_state=1,stratify=t_l)          # test_size=0.25
            ...
            for epoch in range(epochs):
                ...
                if if_adjust_lr:
                    adjust_learning_rate(optimizer, lr, epoch, lr_step=30, lr_gamma=args.adjust_lr_raito)   
                
                train_loss,train_accuracy_all,train_accuracy_imgN,train_accuracy_imgA,train_accuracy_imgL,train_accuracy_cli, = train_a_epoch(model,train_data,all_data,patient_diagnosis_type, patient_Impression_type,batch_size,optimizer,epoch, format_of_coxloss, args)
                
                test_loss,test_accuracy_all,test_accuracy_imgN,test_accuracy_imgA,test_accuracy_imgL,test_accuracy_cli, = prediction(all_data,model,test_data,patient_diagnosis_type, patient_Impression_type,args)  
                val_loss,val_accuracy_all,val_accuracy_imgN,val_accuracy_imgA,val_accuracy_imgL,val_accuracy_cli, = prediction(all_data,model,val_data,patient_diagnosis_type, patient_Impression_type,args)
                if epoch == 0:
                    print("\n")
                ...
                print("epoch:{:2d}, train_accuracy_imgA:{:.4f}, train_accuracy_imgL:{:.4f}, train_accuracy_cli:{:.4f}".format(epoch,train_accuracy_imgA,train_accuracy_imgL,train_accuracy_cli)) 
