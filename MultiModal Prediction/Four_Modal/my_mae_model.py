...
class PretrainVisionTransformerEncoder(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=0, embed_dim=512, depth=12,
                 ...
                 use_learnable_pos_emb=False,train_type_num=4):
        super().__init__()
...
class PretrainVisionTransformerDecoder(nn.Module):
    ...
    def __init__(self, patch_size=16, num_classes=512, embed_dim=512, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=nn.LayerNorm, init_values=None, num_patches=196,train_type_num=4,
                 ):
        super().__init__()
        self.num_classes = num_classes
...
class fusion_model_mae_2(nn.Module):
    def __init__(self,in_feats,n_hidden,out_classes,dropout=0.3,train_type_num=4):  # dropout=0.3
...
#         TransformerConv
        ...
        """
        # 新增一個全連接層用於類別預測
        self.classifier = nn.Linear(out_classes//4, 4) # 假設有 4 種病灶類型
        # 新增模態特定的分類器
        self.classifier_imgN = nn.Linear(out_classes//4, 4)  # 假設有 4 種病灶類型
        self.classifier_imgA = nn.Linear(out_classes//4, 4)
        self.classifier_imgL = nn.Linear(out_classes//4, 4)
        self.classifier_cli = nn.Linear(out_classes//4, 4)
        """

    def forward(self,all_thing,train_use_type=None,use_type=None,in_mask=[],mix=True): # ,mix=False
        # 奇怪
        class_logits_imgN = None
        class_logits_imgA = None
        class_logits_imgL = None
        class_logits_cli = None
        mae_x = None
        ...
        if len(train_use_type)>1:
            ...
            if 'imgA' in train_use_type and 'imgA' in use_type:
                ...
                #print("train_use_type.index('imgA'):"+ str(mae_x[train_use_type.index('imgA')][:5] ))
                k+=1
            if 'imgL' in train_use_type and 'imgL' in use_type:
                x_imgL = x_imgL + mae_x[train_use_type.index('imgL')] 
                #print("train_use_type.index('imgL'):"+ str(mae_x[train_use_type.index('imgL')][:5]))
                k+=1
            if 'cli' in train_use_type and 'cli' in use_type:
                x_cli = x_cli + mae_x[train_use_type.index('cli')]  
                #print("train_use_type.index('cli'):"+ str(mae_x[train_use_type.index('cli')][:5] ))
                k+=1
        ###=========================================================================###      
        ...
        if 'imgN' in data_type:
            ...
            class_logits_imgN = self.classifier_imgN(x_imgN)  # 8-> 4
        ...
        if 'imgA' in data_type:
            ...
            class_logits_imgA = self.classifier_imgA(x_imgA) 
        ...
        if 'imgL' in data_type:
            ...
            class_logits_imgL = self.classifier_imgL(x_imgL) 
        ...
        if 'cli' in data_type:
            ...
            class_logits_cli = self.classifier_cli(x_cli) # 8->4
        ...
        # 使用 'one_x' 進行類別預測
        class_logits_all = self.classifier(one_x)
